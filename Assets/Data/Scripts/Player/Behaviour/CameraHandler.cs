        ...
        public void HandleLockOn()
        {
            ...
            for (int i = 0; i < colliders.Length; i++)
            {
                ...
                if (enemyManager != null && !enemyStats.isDead)
                {
                    ...
                    if (enemyManager.transform.root != targetTransform.root && viewableAngle > -50
                        && viewableAngle < 50 && distanceFromTarget <= maximumLockOnDistance)
                    {
                        if (Physics.Linecast(playerManager.lockOnTransform.position, enemyManager.lockOnTransform.position, out hit))
                        {
                            Debug.DrawLine(playerManager.lockOnTransform.position, enemyManager.lockOnTransform.position);
